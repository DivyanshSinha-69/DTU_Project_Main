...
const BookRecordsPublished = ({ setBlurActive }) => {
  ...
  const fetchBookRecords = async () => {
    try {
      const response = await fetch(
        `https://64.227.135.99:3001/ece/faculty/books/FAC001`,
      );
      if (!response.ok) {
  ...
  const handleAddBook = async (newBook) => {
    ...
    try {
      if (isAddBook) {
        // Add new book record
        const response = await fetch(
          `https://64.227.135.99:3001/ece/faculty/books`,
          {
            method: "POST",
            headers: {
        ...
        const response = await fetch(
          `https://64.227.135.99:3001/ece/faculty/books/${selectedBook.Book_id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDeleteBook = async (Book_id) => {
    const response = await fetch(
      `https://64.227.135.99:3001/ece/faculty/books/${Book_id}`,
      {
        method: "DELETE",
      },
    );
