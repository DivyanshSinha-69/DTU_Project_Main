  ...
  connectDB.query(pdfQuery, [ID], (pdfErr, pdfResult) => {
    ...
    if (pdfResult.length > 0) {
      ...
      if (appointmentLetter) {
        ...
        const relativeFilePath = appointmentLetter.replace('http://18.212.137.222/public', '');
...
export const uploadPdf = (req, res) => {
  upload.single('pdf')(req, res, async (err) => {
    ...
    fs.writeFile(filePath, buffer, (writeErr) => {
      ...
      const baseUrl = 'http://18.212.137.222/public';
...
export const deletePublication = (req, res) => {
  ...
  connectDB.query(pdfQuery, [ID], (pdfErr, pdfResult) => {
    ...
    if (pdfResult.length > 0) {
      ...
      if (manuscript) {
        // Extract the relative file path from the link
        const relativeFilePath = manuscript.replace('http://18.212.137.222/public', '');
...
export const deleteInterInstituteActivity = (req, res) => {
  ...
  connectDB.query(pdfQuery, [ID], (pdfErr, pdfResult) => {
    ...
    if (pdfResult.length > 0) {
      ...
      if (certificate) {
        // Extract the relative file path from the link
        const relativeFilePath = certificate.replace('http://18.212.137.222/public', '');
