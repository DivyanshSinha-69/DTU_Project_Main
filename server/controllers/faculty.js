...
export const addResearchPaper = (req, res) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      const sql =
        "INSERT INTO faculty_research_paper (faculty_id, paper_type, title_of_paper, domain, publication_name, published_date, pdf_path) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
export const updateResearchPaper = (req, res) => {
  ...
  const checkQuery =
    "SELECT * FROM faculty_research_paper WHERE faculty_id = ? AND title_of_paper = ?";
...
export const deleteResearchPaper = (req, res) => {
  ...
  const getQuery =
    "SELECT pdf_path FROM faculty_research_paper WHERE faculty_id = ? AND title_of_paper = ?";
  ...
  pool.query(getQuery, [faculty_id, title_of_paper], (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        message: "No research paper found with the given faculty_id and title.",
      });
    }
    ...
    const deleteQuery =
      "DELETE FROM faculty_research_paper WHERE faculty_id = ? AND title_of_paper = ?";

    pool.query(
      deleteQuery,
      [faculty_id, title_of_paper],
      (err, deleteResult) => {
        if (err) {
          return res.status(500).json({
        ...
        fs.unlink(fullPdfPath, (err) => {
          if (err) {
            return res
              .status(500)
              .json({ message: "Error deleting the PDF file", error: err });
          }
...
export const addFacultyImage = (req, res) => {
  ...
  pool.query(query, [faculty_id, imagePath], (err, result) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error adding faculty image", error: err });
    }

    res
      .status(201)
      .json({ message: "Faculty image added successfully", data: result });
  });
};
...
export const updateFacultyImage = (req, res) => {
  ...
  pool.query(getQuery, [faculty_id], (err, result) => {
    ...
    pool.query(
      updateQuery,
      [faculty_id, newImagePath, newImagePath],
      (err, updateResult) => {
        if (err) {
          return res
            .status(500)
  ...
  pool.query(getQuery, [faculty_id], (err, result) => {
    ...
    fs.unlink(fullImagePath, (err) => {
      ...
      pool.query(deleteQuery, [faculty_id], (err, deleteResult) => {
        if (err) {
          return res.status(500).json({
            message: "Error deleting the faculty image record",
            error: err,
          });
        }
        ...
        if (deleteResult.affectedRows === 0) {
          return res
            .status(404)
            .json({ message: "Failed to delete faculty image record" });
        }

        res
          .status(200)
          .json({ message: "Faculty image and record deleted successfully" });
      });
    });
  });
};
